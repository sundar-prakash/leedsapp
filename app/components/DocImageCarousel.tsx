...
function LeftArrow({ onClick, disabled }: { onClick: () => void; disabled: boolean }) {
  return (
    <button
      className="hidden md:flex embla__button embla__prev absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full shadow-lg p-2"
      onClick={onClick}
      disabled={disabled}
      aria-label="Previous"
      type="button"
...
function RightArrow({ onClick, disabled }: { onClick: () => void; disabled: boolean }) {
  return (
    <button
      className="hidden md:flex embla__button embla__next absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full shadow-lg p-2"
      onClick={onClick}
      disabled={disabled}
      aria-label="Next"
      type="button"
